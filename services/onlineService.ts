...
export const getOnlinePlayers = (
  ...
  return onSnapshot(q, async (snapshot) => {
    const playersFromFirestore = snapshot.docs.map(
      (doc) => doc.data() as OnlinePlayer
    );
    ...
    if (!rtdbSnapshot.exists()) {
      console.warn(
        'Could not fetch RTDB user statuses for liveness check. Displaying all users from Firestore. Some may be stale.'
      );
...
export const claimTimeoutVictory = async (gameId: string, claimant: Player) => {
  ...
  try {
    ...
    if (
      ...
      gameData.currentPlayer !== claimant
    ) {
